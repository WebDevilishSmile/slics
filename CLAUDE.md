...
Error response bodies are inconsistent across routes — some use `{ error }`, others `{ message }` (tracked in `docs/SUGGESTIONS.md` #14). Check the specific route's existing shape before adding a caller.
...
- `app/context/CommentRefreshContext.js` exists but is currently unused/dead code (`docs/SUGGESTIONS.md` #13) — don't assume it's wired up.
...
`docs/SUGGESTIONS.md` is a live, checked-off punch list of security/perf/quality issues found in this codebase — check it before assuming an area is already fixed, and check items off as they're addressed. Notably still open: the SLIC PATCH/DELETE ID-field mismatch (#12), the dead `CommentRefreshContext` (#13), and inconsistent API error shapes (#14). The SLIC create/edit form and its field components live in `app/components/slicForm/` (one `SlicForm` with a `mode` prop); generic fields shared across features (currently `PhoneField`) live in `app/components/form/`.
